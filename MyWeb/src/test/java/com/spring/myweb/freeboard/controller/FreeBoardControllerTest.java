...
//		this.mockMvc = MockMvcBuilders.standaloneSetup(controller).build();
	...
	void testContent() throws Exception {
		...
		System.out.println("ModelMap: " + mv.getModelMap());
		assertEquals("freeboard/freeDetail", mv.getViewName());
	}
	
	@Test
	@DisplayName("3번글의 제목과 내용을 수정한 요청을 post방식으로 전송하면 수정이 진행되고 "
	...
	void testModify() throws Exception {
		...
		String viewPage = mockMvc.perform(MockMvcRequestBuilders.post("/freeboard/modify")
				.param("bno", "3")
				.param("title", "3번 수정된 제목")
				.param("content", "3번 수정된 내용"))
				.andReturn().getModelAndView().getViewName();
		assertEquals("redirect:/freeboard/content?bno=3", viewPage);
	}
